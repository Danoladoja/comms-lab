...
import StudioCohort from '@/components/simulation/StudioCohort';
import MissionLogs from '@/components/simulation/MissionLogs';
    ...
    <StudioLayout>
      <div className="flex-1 flex flex-col lg:flex-row h-full overflow-hidden z-10">
        ...
        {/*
          ...
          For an admin this is the cohort, because the old Mission Log listed
          exercises and an admin sees every exercise — including the private
          copy generated for each learner each time one begins. The count grew
          by one per run and meant nothing, which is why two devices showed two
        ...
        */}
        <div className="lg:w-5/12 flex flex-col bg-[#050b14] relative h-full min-h-0">
          {studioAccess?.isAdmin ? (
            ...
            <MissionLogs simulations={simulations} isLoading={isLoadingSims} programmeById={programmeById} />
          )}
        </div>
      </div>
    </StudioLayout>
